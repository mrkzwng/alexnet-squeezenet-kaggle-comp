...
def cnn_thin_function(features, labels, params, mode):
    '''
    custom convnet
    '''
    ...
    angles = tf.cast(features['angles'], dtype=tf.float32)
    angles = tf.reshape(angles, shape=[-1, 1])
    ...
    conv1_in = reduce1_out
    output_dim = 0
    for layer_id in xrange(conv_layers):
        input_dim = input_dim + output_dim if layer_id > 0 else input_dim
        output_dim = output_dim + conv_chs
    ...
    conv1_in = tf.reshape(conv1_in,
                          shape=[-1, (input_dim+output_dim) * height_width ** 2])
    conv1_in = tf.concat([conv1_in, angles], axis=1)
    W_last = tf.get_variable(name='W_last',
                             shape=[(input_dim+output_dim)*height_width**2 + 1, 128],
                             initializer=tf.contrib.layers.xavier_initializer())
    b_last = tf.Variable(name='b_last',
                         initial_value=tf.random_uniform(
                                shape=[128, ],
                                maxval=0.1))
    activ_last = tf.nn.elu(name='activ_last',
                           features=tf.matmul(conv1_in, W_last) + b_last)
    drop_last = tf.layers.dropout(name='drop_last',
                                  inputs=activ_last,
                                  rate=drop_rate,
                                  training=(mode==tf.estimator.ModeKeys.TRAIN))
    ...
    W_last_last = tf.get_variable(name='W_last_last',
                                  shape=[128, 2],
                                  initializer=tf.contrib.layers.xavier_initializer())
    b_last_last = tf.Variable(name='b_last_last',
                              initial_value=tf.random_uniform(shape=[2, ],
                                                              maxval=0.1))

    logits_sm = tf.matmul(norm_last, W_last_last) + b_last_last
